/**
 * anchor-cloud.js — AnchorBrowser cloud example for Ellucian Banner Student
 *
...
const { login_banner, register_student } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Ellucian Banner Student automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_banner(page, {});
      log('Action 1 result:', data1);

      const data2 = await register_student(page, {});

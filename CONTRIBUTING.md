# Contributing to Ellucian Banner Student Browser Automation
...
- **Broken selectors**: Ellucian Banner Student updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Ellucian Banner Student instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.ELLUCIAN_BANNER_URL}/path/to/section`);
...
When you find working selectors for Ellucian Banner Student, document them:

```javascript
// Ellucian Banner Student selectors (verified YYYY-MM-DD)
...
Ellucian Banner Student updates its UI periodically. When selectors break:
